...
public class Conway {
    ...
    public void set_state(int rows, int cols, Sandbox_Game.Element[][] currentArr,
            Sandbox_Game.Element[][] nextArr) {

        // change each cell state
        for (int r = 0; r < rows; r++) {
            for (int c = 0; c < cols; c++) {

                currentArr[c][r] = nextArr[c][r];
                //  set the visibility
                if (currentArr[c][r] == Sandbox_Game.Element.CONWAY) {
                    Sandbox_Game.grid[c][r] = Sandbox_Game.Element.CONWAY;
                }
